...
router.put('/:id/accept-payment', authMiddleware, async (req, res) => {
  try {
    ...
    // 1. Template lengkap untuk download (public path: /uploads/qr_codes/...)
    // 2. QR polos untuk attachment email (public path juga tersedia)
    const qrData = `SPH-TICKET-${participantId}-${Date.now()}`;
...
router.get('/:id/download-qr-template', authMiddleware, async (req, res) => {
  try {
    ...
    // Generate QR template for download
    const qrData = `SPH-TICKET-${participantId}-${Date.now()}`;
